...
func _process(delta: float) -> void:
	if meta_state != last_meta_state:
		if meta_state == MetaState.StartScreen:
			...
			%BackgroundMusic.stream = preload("res://music/10 Rest in the forest.mp3")
			%BackgroundMusic.play()
		else:
			%StartScreen.visible = false
			%EndScreen.visible = false
...
func start_eruption_countdown():
	%BackgroundMusic.stream = preload("res://music/05 Fight in the forest.mp3")
	%BackgroundMusic.play()

func erupt():
	var now = Time.get_unix_time_from_system()
	game_state.erupted_time_timestamp = now
	%BackgroundMusic.stream = preload("res://music/09 A little boss fight for the road.mp3")
	%BackgroundMusic.play()
